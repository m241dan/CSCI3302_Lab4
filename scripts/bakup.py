# roup members: Andrew Seiler, Nick Elsasser, Justin Yun, Daniel Koris, Brendan Ostrom
...
fig = plt.figure()
img = plt.imshow(world_grid, interpolation='nearest')
plt.draw()
...
def updateFig(*args):
    ...
    for j in range(int(N / cell_size_y)):
        for i in range(int(M / cell_size_x)):
            if ((world_grid[i, j] == [0, 0, 0]).all() or (world_grid[i, j] == [255, 0, 0]).all()):
                if (floor(pose2d_sparki_odometry.x * 100) == i and floor(pose2d_sparki_odometry.y * 100) == j):
                    world_grid[i, j] = [255, 0, 0]
                else:
                    world_grid[i, j] = [0, 0, 0]
    img.set_data(world_grid)
    plt.pause(0.00001)
    return [img]
...
def init():
    ...
    publisher_ping = rospy.Publisher('/sparki/ping_command', Int16)
...
def cell_index_to_ij(cell_index):
    ...
    i = floor(cell_num * SPARKI_SIZE / int((N / cell_size_x))
    j = floor(cell_num * SPARKI_SIZE % int((M / cell_size_y))
